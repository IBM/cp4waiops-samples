...
# Adjust channel setting based on OCP version.
OC_VERSION=$(oc version --output json | jq -r '.openshiftVersion')

MAJOR_VERSION=$(echo "$OC_VERSION" | cut -d'.' -f1)
MINOR_VERSION=$(echo "$OC_VERSION" | cut -d'.' -f2)

echo "[INFO] $(date) Detected OpenShift version: $OC_VERSION"

# Check if version is less than 4.19
if [[ "$MAJOR_VERSION" -lt 4 ]] || [[ "$MAJOR_VERSION" -eq 4 && "$MINOR_VERSION" -lt 19 ]]; then
    CHANNEL=stable-1.4
    echo "[INFO] $(date) Setting OADP subscription channel to $CHANNEL as OCP version is < 4.19"
else
    CHANNEL=stable
    echo "[INFO] $(date) Setting OADP subscription channel to $CHANNEL as OCP version is >= 4.19"
fi

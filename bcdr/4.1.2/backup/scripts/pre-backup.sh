...
postgres_sc=$(oc get pvc -n $namespace -l k8s.enterprisedb.io/cluster -o jsonpath='{.items[0].spec.storageClassName}')
...
if [ "$IS_AIOPS_COMPONENT_ENABLED" = "$V_FALSE" ]; then
   ...
   echo "[INFO] $(date) Before scaling down ir-core-operator-controller-manager deployment replica count is $IRCORE_RC"
   ...
   JSON='{"CASSANDRA_RC": '"$CASSANDRA_RC"', "COUCHDB_RC": '"$COUCHDB_RC"', "MINIO_RC": '"$MINIO_RC"', "IRCORE_RC": '"$IRCORE_RC"',  "REDIS_RC": '"$REDIS_RC"'}'
   rm -f statefulset-rc-data.json
   echo $JSON > statefulset-rc-data.json
   
   #Scaling down ir-core-operator-controller-manager
   oc scale deploy -n $namespace ir-core-operator-controller-manager --replicas=0
   #Scaling down the required pods
   oc scale sts aiops-topology-cassandra c-example-couchdbcluster-m aimanager-ibm-minio c-example-redis-m --replicas=0 -n $namespace
   ...
   CreateBackupPods "c-example-redis-m" "data" $REDIS_RC $namespace "./sts-backup-pod-template.yaml"

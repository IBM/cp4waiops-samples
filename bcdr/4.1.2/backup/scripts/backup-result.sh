...
if [ "$IS_AIOPS_COMPONENT_ENABLED" = "$V_TRUE" ]; then
    ...
    echo "[INFO] $(date) Replica count of statefulset c-example-redis-m is $REDIS_RC"
fi


res=$(velero describe backup $backupName --details -n $velero_namespace |grep  -A10000 -m1 -e 'Restic Backups')
...
while [ $index -lt $MINIO_RC ]; do
    if [[ $res != *"backup-export-aimanager-ibm-minio-$index: backup"* ]]; then
        jq '.minioBackupStatus.velerobackup = false' /tmp/backup-result.json > tmp.$$.json && mv tmp.$$.json /tmp/backup-result.json
        break
    fi
    index=$((index + 1))
done

# Redis velero-backup status update
index=0
if [[ $res == *"backup-data-c-example-redis-m-0: backup"* ]]; then
    jq '.redisBackupStatus.velerobackup = true' /tmp/backup-result.json > tmp.$$.json && mv tmp.$$.json /tmp/backup-result.json
fi
while [ $index -lt $MINIO_RC ]; do
    if [[ $res != *"backup-data-c-example-redis-m-$index: backup"* ]]; then
        jq '.redisBackupStatus.velerobackup = false' /tmp/backup-result.json > tmp.$$.json && mv tmp.$$.json /tmp/backup-result.json
        break
    fi
    index=$((index + 1))
done

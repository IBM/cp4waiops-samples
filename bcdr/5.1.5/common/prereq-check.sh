...
aiopsStatus(){
    ...
    if (kubectl -n $namespace get lifecycleservice aiops --no-headers | grep True); then
	    echo "[INFO] $(date) lifecycleservice is in ready state"
    else
	echo "[ERROR] $(date) lifecycleservice is not in ready state, current status is not ready, hence aborting the execution of further steps!"
        aiopsCheckValue=1
	return $aiopsCheckValue
	#exit 1
    fi
    ...
    asm_status=$(kubectl get asm $asmCR -n $namespace -o go-template='{{range .status.conditions}}{{if eq .type "CassandraReady"}}{{.status}}{{end}}{{if eq .type "DeploymentsReady"}}{{.status}}{{end}}{{end}}') #True True

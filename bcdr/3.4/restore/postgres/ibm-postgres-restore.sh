...
echo $namespace $backupName $ibmPostgresRestoreNamePrefix $ibmPostgresRestoreLabel
...
[ -n "$2" ] && secret=$2
oc get secret -n $namespace $secret -o yaml > /tmp/secret.$uuid || { echo Cannot find secret $secret in current namespace, exit ...; exit -1; }
...
echo Add environment variables and copy file to container ...
cp ../../backup/postgres/backup_restore.sh /tmp/backup_restore.sh.$uuid
...
echo Restore database $dbname from file $dbname.dmp ...
...
{  # try
   ...
   echo "Backup file from  backup-postgres to postgres pod transferred!" 
} || { # catch
   echo "Transfer of backup file to postgres pod failed, hence exiting!" && exit 1
}

# Performing db restore
...
#clean up
oc delete pod backup-postgres -n $namespace
oc delete pvc postgres-backup-data -n $namespace

[ $result -eq 0 ] && echo IBM Postgres restore is completed! The database $dbname was restored. && exit 0
echo Failed to restore database $dbname, exit ... && exit -1

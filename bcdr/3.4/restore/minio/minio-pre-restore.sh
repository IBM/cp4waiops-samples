...
# Deleting previously restored minio resources if exist
...
rm -f minio-rc-data.json
echo $JSON > minio-rc-data.json

# Scaling down the aimanager-ibm-minio sts
oc scale sts aimanager-ibm-minio -n $namespace --replicas=0

# Deleting aimanager-ibm-minio-access-secret
oc get secret  -n $namespace aimanager-ibm-minio-access-secret -o yaml > aimanager-ibm-minio-access-secret-pre-restore.yaml
oc delete secret -n $namespace aimanager-ibm-minio-access-secret

# Deleting all minio pvc's

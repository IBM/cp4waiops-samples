...
if [ -n "$cassandraextrapod" ]; then
    echo "This is multinode cassandra cluster, hence updating the cassandra_functions.sh for other nodes"
    
    #update the cassandra_functions.sh for restore operation and then copying it to other pods of Cassandra in HA cluster
    echo "Updating and copying the cassandra_functions.sh to other Cassandra pods in HA cluster"
    ...
    echo "This is single node Cassandra cluster, so no need to update cassandra_functions.sh"
fi

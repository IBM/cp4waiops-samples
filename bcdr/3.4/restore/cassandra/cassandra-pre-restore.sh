...
echo "Deleting previously restored cassandra resources if exist"
...
echo "Getting the replica count for sts aiops-topology-cassandra before scaling down the required pods"
...
rm -f cassandra-rc-data.json
echo $JSON > cassandra-rc-data.json

echo "Scaling down the aiops-topology-cassandra sts"
oc scale sts aiops-topology-cassandra -n $namespace --replicas=0

echo "Deleting all back-aiops-topology-cassandra pvc's"

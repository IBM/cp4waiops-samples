...
echo "Scaling down the ibm-ir-ai-operator-controller-manager deployment"
...
echo "Saving the original replica count for metric and topology deployments before Cassandra backup"
...
for deployment in $deploymentList; do
    ...
    echo "Replica count for deployment, $deployment before scaling down is $RC"
...
echo "Scaling down the metric and topology deployment before Cassandra backup"
for deployment in $deploymentList; do

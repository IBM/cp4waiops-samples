...
result=$?
[ $result -ne 0 ] && echo Failed to back up database $dbname, exit ... && exit -1
...
echo "Creating postgres backup pod and pvc"
oc create -f postgres-backup.yaml -n $namespace

# Make sure backup-postgres is running
echo " Checking if backup-postgres is running"
...
{  # try
   ...
   echo "Backup file from postgres pod to backup-postgres pod transferred!" 
} || { # catch
   echo "Transfer of backup file to backup-postgres pod failed, hence exiting!"
   echo "Deleting postgres backup pod and pvc"

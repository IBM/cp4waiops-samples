...
RC=$(cat $WORKDIR/restore/cassandra/aiops-topology-rc-data.json | jq -r --arg deployementRC "ibm-ir-ai-operator-controller-manager-RC" '.[$deployementRC]')
...
for deployment in $deploymentList; do
    ...
    {
    #TRY
        RC=$(cat $WORKDIR/restore/cassandra/aiops-topology-rc-data.json | jq -r --arg deployementRC "$deployment-RC" '.[$deployementRC]')

...
ASM_USER=$(oc -n $namespace get secret aiops-topology-asm-credentials -o jsonpath="{.data['username']}" | base64 -d)
ASM_PASS=$(oc -n $namespace get secret aiops-topology-asm-credentials -o jsonpath="{.data['password']}" | base64 -d)

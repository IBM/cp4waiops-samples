...
echo "[INFO] $(date) Removing previously registered backup path from elastisearch CR if exists"
...
oc patch --type=json elasticsearches.elastic.automation.ibm.com iaf-system -n $namespace -p="[{ 'op': 'remove','path': '/spec/nodegroupspecs/0/config/$INDEX'}]" 2> /dev/null

echo "[INFO] $(date) Removing previously registered snapshot location from elastisearch CR if exists"
oc patch --type=json elasticsearches.elastic.automation.ibm.com iaf-system -n $namespace -p="[{ 'op': 'remove','path': '/spec/snapshotStores'}]" 2> /dev/null
...
echo "[INFO] $(date) Waiting till all elasticsearch pods are READY after removing backup path and snapshot location from elasticsearch CR"
...
echo "[INFO] $(date) Updating backup path and snapshot location in elasticsearch CR"
$WORKDIR/restore/elasticsearch/elasticsearch-cr-update.sh

...
stopAiTraining() {
    ...
    encoded_username=$(kubectl get secret $es_cred_secret -o json -n $namespace | jq -r '.data.username')
    encoded_password=$(kubectl get secret $es_cred_secret -o json -n $namespace | jq -r '.data.password')

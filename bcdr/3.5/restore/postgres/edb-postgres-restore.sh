...
[ -n "$1" ] && secret=$1
oc -n $ns get secret $installation-edb-secret -o yaml > /tmp/secret1.$uuid || { echo "[ERROR] $(date) Cannot find secret $installation-edb-secret in namespace $ns, hence exiting!"; ./postgres-post-restore.sh; exit -1; }
oc -n $ns get secret $secret -o yaml > /tmp/secret2.$uuid || { echo "[ERROR] $(date) Cannot find secret $secret in namespace $ns, hence exiting!"; ./postgres-post-restore.sh; exit -1; }
pod=$(oc -n $ns get clusters.postgresql.k8s.enterprisedb.io | grep -v '^NAME' | sed -e 's/.* //')
[ -n "$2" ] && pod=$2
oc -n $ns get pod $pod -o yaml > /tmp/server.$uuid || { echo "[ERROR] $(date) Cannot find pod $pod in namespace $ns hence exiting!"; ./postgres-post-restore.sh; exit -1; }
...
cp ../../backup/scripts/postgres/edb_backup_restore.sh /tmp/edb_backup_restore.sh.$uuid
...
{  # try
   ...
   echo "[ERROR] $(date) Transfer of backup file to postgres pod failed, hence exiting!" && ./postgres-post-restore.sh && exit 1
}

...
[ -n "$2" ] && secret=$2
oc get secret -n $namespace $secret -o yaml > /tmp/secret.$uuid || { echo "[ERROR] $(date) Cannot find secret $secret in current namespace hence exiting!"; ./postgres-post-restore.sh; exit -1; }

pod=cp4waiops-postgres-keeper-0
[ -n "$3" ] && pod=$3
oc get pod $pod -n $namespace -o yaml > /tmp/server.$uuid || { echo "[ERROR] $(date) Cannot find pod $pod in current namespace hence exiting!"; ./postgres-post-restore.sh; exit -1; }
...
cp ../../backup/scripts/postgres/backup_restore.sh /tmp/backup_restore.sh.$uuid

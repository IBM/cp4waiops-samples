...
rm -f automationbase-cr-patch-copy.json

cp automationbase-cr-patch.json automationbase-cr-patch-copy.json

# Updating automationbase cr for elasticsearch backup
sed -i "s/STORAGE_CLASS/$rwx_sc/g" automationbase-cr-patch-copy.json
...
oc patch automationbase automationbase-sample  -n $namespace --patch-file automationbase-cr-patch-copy.json --type=merge

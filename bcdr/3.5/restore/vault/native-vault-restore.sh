...
folder=ibm-vault-deploy-backup
# [ -n "$1" ] && folder=$1
deploy_cr=$(echo $folder | sed -e 's/-backup$//')
...
[ ! -e ibm-vault-access-vault-credential.yaml ] && echo Cannot find the file ibm-vault-access-vault-credential.yaml, exit ... && exit -1

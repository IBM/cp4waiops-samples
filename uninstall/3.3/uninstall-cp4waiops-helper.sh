...
delete_securetunnel(){
    ...
    log $INFO "Deleting TLS certificate secret generated by cert manager..."
    oc get secrets -n $CP4WAIOPS_PROJECT --no-headers -o name | grep 'sre-tunnel[0-9|a-z|-]*cert' | while read a b; do oc delete "$a" -n $CP4WAIOPS_PROJECT --ignore-not-found

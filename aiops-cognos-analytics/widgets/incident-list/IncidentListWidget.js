...
class Renderer extends BaseRenderer {
  constructor(options) {
    ...
    this.content.on('change:property', (data) => {
      ...
      if (data?.info?.events[0].name === 'change:property:dropdownFilter') {
        ...
        iframeIncidentListWidget.contentWindow.postMessage({ source: 'dashboard', params: { filtername: filterName } }, this.proxyHost);
      }

      if (data?.info?.events[0].name === 'change:property:dropdownView') {
        const viewName = data?.info?.events[0].info.value;
        iframeIncidentListWidget.contentWindow.postMessage({ source: 'dashboard', params: { viewname: viewName } }, this.proxyHost);
      }

      if (data?.info?.events[0].name === 'change:property:toggleToolbar') {
        const showToolbar = data?.info?.events[0].info.value;
        iframeIncidentListWidget.contentWindow.postMessage({ source: 'dashboard', params: { showtoolbar: showToolbar } }, this.proxyHost);
      }

      if (data?.info?.events[0].name === 'change:property:toggleDetails') {
        const showDetails = data?.info?.events[0].info.value;
        iframeIncidentListWidget.contentWindow.postMessage({ source: 'dashboard', params: { showdetails: showDetails } }, this.proxyHost);
      }
  ...
  renderControl() {
    ...
    this.parentNode.innerHTML = "<iframe id='" + this.iframeId + "' src='" + this.proxyHost + '/aiops/default/widgets/incidents?showtoolbar=' +
      showToolbar +
      '&showdetails=' +
      showDetails +
      '&filtername=' +

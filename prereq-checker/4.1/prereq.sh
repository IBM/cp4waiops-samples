...
# 'AI Manager - Hardware requirements' https://ibm.biz/aiops_hardware_411
    ...
    spec:
      ...
      template:
        ...
        spec:
          ...
          containers:
          - name: testimage
            image: cp.icr.io/cp/cp4waiops/ai-platform-api-server@sha256:98840b62d0a5887be10e8877ac658bb0f7a9e292af775751038e5e4e25b6f036
...
function checkIBMSpectrum {
  ...
  if [[ "$OCP_VER" == *"4.10"* || "$OCP_VER" == *"4.12"* ]]; then
    ...
    if [[ "$?" == "1" ]]; then
      printf "${fail_color}${ERROR} StorageClass ibm-spectrum-scale-sc does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_411 for details.$color_end\n"
...
function checkODF {
  ...
  for s in "${ODF_STORAGECLASSES[@]}"; do
    ...
    if [[ "$?" == "1" ]]; then
      printf " $fail_color $ERROR StorageClass $s does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_411 for details.$color_end\n"
      ODF_VE_FLAG="true"
    fi
  done
...
function checkPortworx {
  ...
  if [[ "$?" == "0" ]]; then
    ...
    echo
    log $WARNING "StorageClass \"portworx-fs\" does not exist. See \"Portworx Storage\" section in https://ibm.biz/storage_consideration_411 for details.\n"
  fi
  ...
  if [[ "$?" == "0" ]]; then
    ...
    PORTWORX_WARNING="true"
    echo
    log $WARNING "StorageClass \"portworx-aiops\" does not exist. See \"Portworx Storage\" section in https://ibm.biz/storage_consideration_411 for details.\n"
  fi
  
  portworx_fs=$(checkAllowVolumeExpansion portworx-aiops)
  if [[ "$?" == "1" ]]; then
    echo
    printf " $fail_color $ERROR StorageClass portworx-aiops does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_411 for details.$color_end\n"
    storageCheckRes+=("fail")
    return 1
  fi
  ...
  if [[ "$?" == "1" ]]; then
    echo
    printf " $fail_color $ERROR StorageClass portworx-fs does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_411 for details.$color_end\n"
    storageCheckRes+=("fail")
    return 1
  fi
...
function checkIBMCFileGoldGidStorage {
  ...
  if [[ "$file" == "" || "$block" == "" ]]; then
    printf "$fail_color $ERROR Both ibmc-block-gold and ibmc-file-gold-gid need to exist to use IBM Cloud Storage. See \"Storage\" section in https://ibm.biz/storage_consideration_411 for details. $color_end\n"
    storageCheckRes+=("fail")
    return 1 
  fi
  ...
  if [[ "$?" == "1" ]]; then
    printf " $fail_color $ERROR StorageClass ibmc-block-gold does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_411 for details.$color_end\n"
    storageCheckRes+=("fail")
    return 1
  fi
  ...
  if [[ "$?" == "1" ]]; then
    printf " $fail_color $ERROR StorageClass ibmc-file-gold-gid does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_411 for details.$color_end\n"
    storageCheckRes+=("fail")
    return 1
  fi
...
function checkStorage {
  ...
  if [ ${#storageFound[@]} -eq 0 ]; then
    ...
    printf "$fail_color $ERROR The supported Storage Providers are Portworx, Openshift Data Foundation, IBM Cloud Storage for ROKS, or IBM Spectrum Fusion/IBM Spectrum Scale Container Native. See https://ibm.biz/storage_consideration_411 for details.$color_end\n"
    STORAGE_PROVIDER_RES=$fail_msg
    startEndSection "Storage Provider"
    return 1
  fi

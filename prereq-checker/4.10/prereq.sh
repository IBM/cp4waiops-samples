...
display_help() {
    echo "**************************************** Usage ********************************************"
    echo
    echo " This script ensures that you have met the technical prerequisites for IBM Cloud Pak for AIOps version 4.10."
    echo
...
echo
log $INFO "Starting IBM Cloud Pak for AIOps prerequisite checker v4.10..." ""
echo
...
function checkIBMSFHCI {
    ...
    if [[ "$?" == "1" ]]; then
        log $ERROR "StorageClass ibm-storage-fusion-cp-sc does not have allowedVolumeExpansion enabled. This is required for all Production sized installs and strongly recommended for starter sized installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
...
function checkIBMStorageFusion {
    ...
    if [[ "$OCP_VER" == *"4.14"* || "$OCP_VER" == *"4.15"* || "$OCP_VER" == *"4.16"* || "$OCP_VER" == *"4.17"*  ]]; then
        ...
        if [[ "$?" == "1" ]]; then
            log $ERROR "StorageClass ibm-spectrum-scale-sc does not have allowedVolumeExpansion enabled. This is required for all Production sized installs and strongly recommended for starter sized installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
            storageCheckRes+=("fail")
            return 1
        fi
        ...
        log $ERROR "If you intend to use Storage Fusion with AIOPS 4.10, you must have OCP 4.14, 4.15, or 4.16" $fail_color
...
function checkODF {
    ...
    for s in "${ODF_STORAGECLASSES[@]}"; do
        ...
        if [[ "$?" == "1" ]]; then
            log $ERROR "StorageClass $s does not have allowedVolumeExpansion enabled. This is required for all Production sized installs and strongly recommended for starter sized installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
            ODF_VE_FLAG="true"
        fi
    done
...
function checkNonCSIPortworx {
    ...
    if [[ "$?" == "0" ]]; then
        ...
        log $ERROR "No valid Portworx StorageClass found. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." 
        return 1
    fi
    ...
    if [[ "$?" == "0" ]]; then
        ...
        log $ERROR "No valid Portworx StorageClass found. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
        return 1
    fi
    ...
    if [[ "$?" == "1" ]]; then
        echo
        log $ERROR "StorageClass portworx-aiops does not have allowedVolumeExpansion enabled. This is required for all Production sized installs and strongly recommended for starter sized installs. See the \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
        return 1
    fi
    ...
    if [[ "$?" == "1" ]]; then
        echo
        log $ERROR "StorageClass portworx-fs does not have allowedVolumeExpansion enabled. This is required for all Production sized installs and strongly recommended for starter sized installs. See the \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
        return 1
    fi
...
function checkPortworx {
    ...
    if [[ "$?" == "0" ]]; then
        ...
        if [[ "$?" == "1" ]]; then
            echo
            log $ERROR "StorageClass px-csi-aiops does not have allowedVolumeExpansion enabled. This is required for all Production sized installs and strongly recommended for starter sized installs. See the \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
            storageCheckRes+=("fail")
            return 1
        fi
    else
        ...
        if [[ "$?" == "0" ]]; then
            ...
            if [[ "$?" == "1" ]]; then
                echo
                log $ERROR "StorageClass px-csi-aiops-mz does not have allowedVolumeExpansion enabled. This is required for all Production sized installs and strongly recommended for starter sized installs. See the \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
                storageCheckRes+=("fail")
                return 1
            fi
        else
...
function checkIBMCFileGoldGidStorage {
    ...
    if [[ "$file" == "" || "$block" == "" ]]; then
        log $ERROR "Both ibmc-block-gold and ibmc-file-gold-gid need to exist to use IBM Cloud Storage. See \"Storage\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
        storageCheckRes+=("fail")
        return 1 
    fi
    ...
    if [[ "$?" == "1" ]]; then
        log $ERROR "StorageClass ibmc-block-gold does not have allowedVolumeExpansion enabled. This is required for all Production sized installs and strongly recommended for starter sized installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
        storageCheckRes+=("fail")
        return 1
    fi
    ...
    if [[ "$?" == "1" ]]; then
        log $ERROR "StorageClass ibmc-file-gold-gid does not have allowedVolumeExpansion enabled. This is required for all Production sized installs and strongly recommended for starter sized installs. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_410 for details." $fail_color
        storageCheckRes+=("fail")
        return 1
    fi
...
function checkAWS {
    printf "Checking if AWS Native Storage is configured properly...\n" | tee -a $res_file

    gp3=$(${CLI} get storageclass gp3-csi --ignore-not-found=true)
    efs=$(${CLI} get storageclass efs-sc --ignore-not-found=true)

    # AWS Native storage needs gp3-csi to work with aiops
    if [[ "$gp3" == "" ]]; then
        log $ERROR "AWS Native Storage will need gp3-csi to function with CP4AIOPS. See \"Storage Class Requirements\" section in https://ibm.biz/storage_consideration_4100 for details." $fail_color
        storageCheckRes+=("fail")
        return 1
    fi

    if [[ "$gp3" != "" && "$efs" != "" ]]; then
        log $INFO "Both gp3-csi and efs-sc storageclasses are available. You may use AWS native storage (with OADP backup and restore)." ""
    elif [[ "$gp3" != "" && "$efs" == "" ]]; then
        log $INFO "Only gp3-csi storageclass is available. You may use AWS native storage (without OADP backup and restore)." ""
    fi
...
function checkStorage {
    ...
    # Check for AWS
    AWS_GP3=$(${CLI} get storageclass gp3-csi --ignore-not-found=true)
    AWS_EFS=$(${CLI} get storageclass efs-sc --ignore-not-found=true)
    if [[ "$AWS_GP3" != "" || "$AWS_EFS" != "" ]]; then
        echo
        log $INFO "A storage class related to AWS native storage was found." ""
        storageFound+=("aws")
    else
        log $INFO "No AWS native storage found... Skipping configuration check."
    fi
    ...
    if [[ "$IBM_SF_HCI" != "" ]]; then
        ...
        log $INFO "No IBM Storage Fusion HCI System storage... Skipping configuration check." ""
    fi
    ...
    # Check for ODF...
    ODF_PODS=($(${CLI} get pods -n openshift-storage --no-headers=true | grep -v 'storageclient-.*-status-report' | awk '{print $1}'))
    if [[ "$ODF_PODS" == "" ]]; then
        echo
    ...
    if [ ${#storageFound[@]} -eq 0 ]; then
        ...
        log $ERROR "The supported Storage Providers are Portworx, Openshift Data Foundation, IBM Cloud Storage for ROKS, or IBM Spectrum Fusion/IBM Spectrum Scale Container Native. See https://ibm.biz/storage_consideration_410 for details." $fail_color
        STORAGE_PROVIDER_RES=$fail_msg
        startEndSection "Storage Provider"
        return 1
    fi
...
function main {
    ...
    log $WARNING "As of v4.10.1, IBM Cloud Pak for AIOps has deprecated the prerequisite checker script, which is no longer supported. To check prerequisites, please use the aiopsctl plugin." $warn_color

...
display_help() {
   echo "**************************************** Usage ********************************************"
   echo
   echo " This script ensures that you have met the technical prerequisites for IBM Cloud Pak for AIOps version 3.6."
   echo
...
echo
log $INFO "Starting IBM Cloud Pak for Watson AIOps AI Manager prerequisite checker v3.6..."
echo
    ...
    spec:
      ...
      template:
        ...
        spec:
          ...
          containers:
          - name: testimage
            image: cp.icr.io/cp/cp4waiops/ai-platform-api-server@sha256:1f41373631645c47d5d66ed6033ee3d7b20a7198eccf1a8f7d3eff8b3dea74e3
...
function checkPortworx {
  ...
  if [[ "$?" == "0" ]]; then
    ...
    echo
    log $WARNING "StorageClass \"portworx-fs\" does not exist. See \"Portworx Storage\" section in https://ibm.biz/storage_consideration_360 for details.\n"
  fi
  ...
  if [[ "$?" == "0" ]]; then
    ...
    PORTWORX_WARNING="true"
    echo
    log $WARNING "StorageClass \"portworx-aiops\" does not exist. See \"Portworx Storage\" section in https://ibm.biz/storage_consideration_360 for details.\n"
  fi
...
if [[ $worker_node_count -ge $NODE_COUNT_LARGE_3_6 ]]; then
...
if [[ $total_cpu_unrequested -ge $VCPU_LARGE_3_6 ]]; then
...
if [[ $total_memory_unrequested_GB -ge $MEMORY_LARGE_3_6 ]]; then
...
if [[ $worker_node_count -ge $NODE_COUNT_SMALL_3_6 ]]; then
...
if [[ $total_cpu_unrequested -ge $VCPU_SMALL_3_6 ]]; then
...
if [[ $total_memory_unrequested_GB -ge $MEMORY_SMALL_3_6 ]]; then
...
checkSmallOrLargeProfileInstall() {
  ...
  log $INFO "${header}"
  string=`printf "Small profile(available/required)  [ %s/ %s ]   [ %s/ %s ]       [ %s/ %s ]" "$small_worker_node_count_string" "$NODE_COUNT_SMALL_3_6" "$small_total_cpu_unrequested_string" "$VCPU_SMALL_3_6" "$small_total_memory_unrequested_GB_string" "$MEMORY_SMALL_3_6"`
  log $INFO "${string}"
  string=`printf "Large profile(available/required)  [ %s/ %s ]   [ %s/ %s ]       [ %s/ %s ]" "$large_worker_node_count_string" "$NODE_COUNT_LARGE_3_6" "$large_total_cpu_unrequested_string" "$VCPU_LARGE_3_6" "$large_total_memory_unrequested_GB_string" "$MEMORY_LARGE_3_6"`
  log $INFO "${string}"
  ...
  if [[ ($worker_node_count -ge $NODE_COUNT_LARGE_3_6) && ($total_cpu_unrequested -ge $VCPU_LARGE_3_6) && ($total_memory_unrequested_GB -ge $MEMORY_LARGE_3_6)  ]] ; then
     log $INFO "Cluster currently has resources available to create a large profile of Cloud Pak for Watson AIOps AI Manager"
  elif [[ $worker_node_count -ge $NODE_COUNT_SMALL_3_6 && ($total_cpu_unrequested -ge $VCPU_SMALL_3_6) && ($total_memory_unrequested_GB -ge $MEMORY_SMALL_3_6) ]] ; then

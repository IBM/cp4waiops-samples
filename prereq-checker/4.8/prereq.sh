...
# Minimum resource values for Production sized 4.8.x (base install)
...
display_help() {
    echo "**************************************** Usage ********************************************"
    echo
    echo " This script ensures that you have met the technical prerequisites for IBM Cloud Pak for AIOps version 4.8."
    echo
...
echo
log $INFO "Starting IBM Cloud Pak for AIOps prerequisite checker v4.8..."
echo
    ...
    if [[ "${isOpenshift}" == "0" ]]; then
        ...
        if [[ $OCP_VER == *"4.16"* ]]; then
            if [[ $OCP_MINOR_VER -ge 4 ]]; then
                printf "$pass_color$INFO OCP version $OCP_VER is compatible but only nodes with x86_64 (amd64) architectures are supported at this time. $color_end\n"
                OCP_VER_RES=$pass_msg
                startEndSection "Platform Version Check"
                return 0
            else
                printf "$fail_color$ERROR OCP Version is incompatible. Required Versions: v4.14, v4.15, v4.16.4+. $color_end\n"
...
function checkIBMStorageFusion {
    ...
    if [[ "$OCP_VER" == *"4.14"* || "$OCP_VER" == *"4.15"* || "$OCP_VER" == *"4.16"* ]]; then
        ...
        printf "${fail_color}${ERROR}If you intend to use Storage Fusion with AIOPS 4.8, you must have OCP 4.14, 4.15, or 4.16 $color_end\n"

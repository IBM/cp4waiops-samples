...
## Resource size for 4.3
# These defaults are given in section 'IBM Cloud Pak for AIOps only Hardware requirement totals' under
# 'Hardware requirements - IBM Cloud Pak for AIOps only' https://ibm.biz/aiops_hardware_430
# Minimum resource values for small profile 4.3.x
NODE_COUNT_SMALL_4_3=3
VCPU_SMALL_4_3=62
MEMORY_SMALL_4_3=140
# Minimum resource values for large profile 4.3.x
NODE_COUNT_LARGE_4_3=10
VCPU_LARGE_4_3=162
MEMORY_LARGE_4_3=372
...
display_help() {
    echo "**************************************** Usage ********************************************"
    echo
    echo " This script ensures that you have met the technical prerequisites for IBM Cloud Pak for AIOps version 4.3."
    echo
...
echo
log $INFO "Starting IBM Cloud Pak for AIOps prerequisite checker v4.3..."
echo
    ...
    spec:
      ...
      template:
        ...
        spec:
          ...
          containers:
          - name: testimage
            image: cp.icr.io/cp/cp4waiops/ai-platform-api-server@sha256:a9f05323e16d3779dfeff262b3213529471595e05fe134ca87c90ba1fccbba61
...
function checkIBMSpectrum {
    ...
    if [[ "$OCP_VER" == *"4.12"* ]]; then
        ...
        if [[ "$?" == "1" ]]; then
            printf "${fail_color}${ERROR} StorageClass ibm-spectrum-scale-sc does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in ttps://ibm.biz/storage_consideration_430 for details.$color_end\n"
        ...
        printf "${fail_color}${ERROR}If you intend to use Storage Fusion with AIOPS 4.3, you must have 4.12 $color_end\n"
...
function checkODF {
    ...
    for s in "${ODF_STORAGECLASSES[@]}"; do
        ...
        if [[ "$?" == "1" ]]; then
            printf " $fail_color $ERROR StorageClass $s does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in ttps://ibm.biz/storage_consideration_430 for details.$color_end\n"
            ODF_VE_FLAG="true"
        fi
    done
...
function checkPortworx {
    ...
    if [[ "$?" == "0" ]]; then
        ...
        echo
        log $WARNING "StorageClass \"portworx-fs\" does not exist. See \"Portworx Storage\" section in ttps://ibm.biz/storage_consideration_430 for details.\n"
    fi
    ...
    if [[ "$?" == "0" ]]; then
        ...
        PORTWORX_WARNING="true"
        echo
        log $WARNING "StorageClass \"portworx-aiops\" does not exist. See \"Portworx Storage\" section in ttps://ibm.biz/storage_consideration_430 for details.\n"
    fi
    
    portworx_fs=$(checkAllowVolumeExpansion portworx-aiops)
    if [[ "$?" == "1" ]]; then
        echo
        printf " $fail_color $ERROR StorageClass portworx-aiops does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in ttps://ibm.biz/storage_consideration_430 for details.$color_end\n"
        storageCheckRes+=("fail")
        return 1
    fi
    ...
    if [[ "$?" == "1" ]]; then
        echo
        printf " $fail_color $ERROR StorageClass portworx-fs does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in ttps://ibm.biz/storage_consideration_430 for details.$color_end\n"
        storageCheckRes+=("fail")
        return 1
    fi
...
function checkIBMCFileGoldGidStorage {
    ...
    if [[ "$file" == "" || "$block" == "" ]]; then
        printf "$fail_color $ERROR Both ibmc-block-gold and ibmc-file-gold-gid need to exist to use IBM Cloud Storage. See \"Storage\" section in ttps://ibm.biz/storage_consideration_430 for details. $color_end\n"
        storageCheckRes+=("fail")
        return 1 
    fi
    ...
    if [[ "$?" == "1" ]]; then
        printf " $fail_color $ERROR StorageClass ibmc-block-gold does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in ttps://ibm.biz/storage_consideration_430 for details.$color_end\n"
        storageCheckRes+=("fail")
        return 1
    fi
    ...
    if [[ "$?" == "1" ]]; then
        printf " $fail_color $ERROR StorageClass ibmc-file-gold-gid does not have allowedVolumeExpansion enabled. This is required for all large profile installs and strongly recommended for small profile installs. See \"Storage Class Requirements\" section in ttps://ibm.biz/storage_consideration_430 for details.$color_end\n"
        storageCheckRes+=("fail")
        return 1
    fi
...
function checkStorage {
    ...
    if [ ${#storageFound[@]} -eq 0 ]; then
        ...
        printf "$fail_color$ERROR The supported Storage Providers are Portworx, Openshift Data Foundation, IBM Cloud Storage for ROKS, or IBM Spectrum Fusion/IBM Spectrum Scale Container Native. See ttps://ibm.biz/storage_consideration_430 for details.$color_end\n"
        STORAGE_PROVIDER_RES=$fail_msg
        startEndSection "Storage Provider"
        return 1
    fi
...
analyze_resource_display() {
    ...
    elif [[ $worker_node_count -ge $NODE_COUNT_LARGE_4_3 ]]; then
    ...
    if [[ $total_cpu_unrequested -ge $VCPU_LARGE_4_3 ]]; then
    ...
    if [[ $total_memory_unrequested_GB -ge $MEMORY_LARGE_4_3 ]]; then
    ...
    if [[ $worker_node_count -ge $NODE_COUNT_SMALL_4_3 ]]; then
    ...
    if [[ $total_cpu_unrequested -ge $VCPU_SMALL_4_3 ]]; then
    ...
    if [[ $total_memory_unrequested_GB -ge $MEMORY_SMALL_4_3 ]]; then
...
checkSmallOrLargeProfileInstall() {
    ...
    log $INFO "${header}"
    string=`printf "Small profile(available/required)  [ %s/ %s ]   [ %s/ %s ]       [ %s/ %s ]" "$small_worker_node_count_string" "$NODE_COUNT_SMALL_4_3" "$small_total_cpu_unrequested_string" "$VCPU_SMALL_4_3" "$small_total_memory_unrequested_GB_string" "$MEMORY_SMALL_4_3"`
    log $INFO "${string}"
    string=`printf "Large profile(available/required)  [ %s/ %s ]   [ %s/ %s ]       [ %s/ %s ]" "$large_worker_node_count_string" "$NODE_COUNT_LARGE_4_3" "$large_total_cpu_unrequested_string" "$VCPU_LARGE_4_3" "$large_total_memory_unrequested_GB_string" "$MEMORY_LARGE_4_3"`
    log $INFO "${string}"
    ...
    if [[ ("$largeProfileNodeCheck" == "true" ) && ($total_cpu_unrequested -ge $VCPU_LARGE_4_3) && ($total_memory_unrequested_GB -ge $MEMORY_LARGE_4_3)  ]] ; then
        log $INFO "Cluster currently has resources available to create a large profile of Cloud Pak for AIOps"
    elif [[ $worker_node_count -ge $NODE_COUNT_SMALL_4_3 && ($total_cpu_unrequested -ge $VCPU_SMALL_4_3) && ($total_memory_unrequested_GB -ge $MEMORY_SMALL_4_3) ]] ; then
